...
SIM_UNIT = 1.2;	# update every 0.3 seconds for step size = 1 -> 70 mph
                # update every 0.9 seconds for step size = 2 -> 50 mph
                # update every 0.6 seconds for step size = 1 -> 35 mph
NO_VEH = 100;
RUN_TIME = 2025; # want to use up whole trajectory file
...
f = open("urban81by81_lg_new.txt", "r");
line = f.readline();
s = line.split()
...
#g = open("grid_urban_general.out","w");
h = open("urban_81by81_lg.out","w");
...
while int(t.time()) < stopTime :
  ...
  if (lastUpdate + updateRate < t.time() ):   # time to update
    print "Time to update! it's now ", int(t.time()/t.ticksPerSecond()), ", next update at ", str((updateRate+t.time())/t.ticksPerSecond())
